...
async function fetchLedger(shopId: string, month: string): Promise<ShopLedger> {
  const res = await api.api.v1.khata[":shopId"].$get({ param: { shopId }, query: { month } });
...
function BalanceBadge({ balance }: { balance: number }) {
  if (balance > 0) return <Badge variant="destructive">Owes ৳{balance.toLocaleString()}</Badge>;
  if (balance < 0) return <Badge variant="secondary">Overpaid ৳{Math.abs(balance).toLocaleString()}</Badge>;
          ...
          <div className="rounded-lg border p-4 mb-4 space-y-1 text-sm">
            ...
            <div className="border-t pt-1 flex justify-between font-semibold">
              ...
              <span className={data.outstandingBalance > 0 ? "text-destructive" : "text-green-600"}>
                {data.outstandingBalance > 0 ? "+" : ""}৳{data.outstandingBalance.toLocaleString()}
              </span>
            </div>
          </div>
        )}
            ...
            >
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {entry.type === "order" ? `Order #${entry.orderNumber}` : "Payment"}
                  </span>
              ...
              <div className="text-right space-y-0.5">
                <p className={entry.type === "order" ? "text-destructive font-medium" : "text-green-600 font-medium"}>
        ...
        <div className="space-y-2">
          {overview.map((shop) => (
            <button
              key={shop.shopId}
              type="button"
              onClick={() => setSelectedShop({ id: shop.shopId, name: shop.shopName })}
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Ordered: ৳{shop.totalOrdered.toLocaleString()} · Paid: ৳{shop.totalPaid.toLocaleString()}
                  </p>
                </div>
